...
function validateOrder(OrderRequest orderRequest) returns error? {
    Product[] filteredOrders = products.filter(p => p.id == orderRequest.productId);
    if filteredOrders.length() == 0 {
        return error("Product not found");
    }

    Product product = filteredOrders[0];
...
function updateStock(string productId, int quantity) {
    foreach int i in 0 ..< products.length() {
        if products[i].id == productId {
            products[i] = {
                id: products[i].id,
                price: products[i].price,
                stock: products[i].stock - quantity
            };
            break;
        }
    }
